...
const manProfiles = [
    ...
    {name: 'Jacob', hobbies: ['skydiving', 'rock climbing', 'surfing', 'hiking']},
...
const womanProfiles = [
    ...
    {name: 'Michelle', hobbies: ['running', 'gardening', 'cooking', 'watching movies']},
    {name: 'Claire', hobbies: ['surfing', 'swimming', 'diving', 'kayaking']}
]

const getManProfile = (name) => new Promise((resolve, reject) => {
    setTimeout(() => {
        let man = manProfiles.find(profile => name.toLowerCase() === profile.name.toLowerCase())
...
const getWomanProfile = (name) => new Promise((resolve, reject) => {
    setTimeout(() => {
        let woman = womanProfiles.find(profile => name.toLowerCase() === profile.name.toLowerCase())
...
const checkHobbies = (manHobbies, womanHobbies) => new Promise((resolve, reject) => {
    let sameHobbies = manHobbies.filter(hobby => womanHobbies.includes(hobby) )
...
async function checkMatch(manName, womanName) {
    try {
        ...
        let matchResult = await checkHobbies(manProfile.hobbies, womanProfile.hobbies)

        console.log(`${matchResult.msg} Your same hobbies: ${matchResult.sameHobbies}`)
    } catch (error) {
        console.log(error)
    }
}
...
checkMatch('caleb', 'emely')


// Program 2: Random Chat Response

const chatResponses = ['Eh gimana gimana?', 'Oh', 'Oh gitu ya', 'Mantappp', 'Y', 'Hehe',
                        'Anjaaayyy', 'Apaan sih', 'Kok bisa?', 'Wkwkwk', 'Siapa ya?']

const getChatResponse = (chat) => new Promise((resolve, reject) => {
    console.log(chat)
...
getChatResponse('Hei kamu').then(result => {
    console.log(result)
}).catch(error => {
    console.log(error)
})